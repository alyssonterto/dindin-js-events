...
btnEnviar.onclick = (evento) => {
  ...
  evento.preventDefault(); // impede que a pagina seja recarregada / previne comportamento padrão
  ...
  let novaReceita = {
    ...
    ingredientes: inputIngredientes.value.split(','), //split -> virgula é o separador; é tipo o tokenat e a "," é o separador
...
function exibirReceitas() {
  ...
  for (let index = 0; index < listaDeReceitas.length; index++) { //+= mantem o conteudo q ja tem e adiciona um novo
    // ${} = add uma variavel, objeto etc a tag, sem ele só adicionaria o texto
    htmlReceitas += 
      `<article class="card"> 
          <h2>${listaDeReceitas[index].titulo}</h2> 
          <h5>Dificuldade: ${listaDeReceitas[index].dificuldade}</h5 >
          <h5>Preparo: ${listaDeReceitas[index].preparo}</h5 >
          <h5>Ingredientes: ${listaDeReceitas[index].ingredientes}</h5 >
          <h5>Link: ${listaDeReceitas[index].link}</h5 >
          <h5>Vegano? T/F: ${listaDeReceitas[index].vegano}</h5>
      </article>`;
  }
...
exibirReceitas(); //ao startat a page exibir a receita, e ao clicar no botao cadastrar exibir tambem
